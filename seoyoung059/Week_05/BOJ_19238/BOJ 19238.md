...
- 벽과 빈칸밖에 없으므로, 승객의 위치는 map에 표시한다.
    - 이 때 표시를 -승객번호로 하여 승객 번호를 확인할 수 있으면서도 벽과 구분지을 수 있도록 했다.
- 승객을 찾는 것과 목적지로 이동하는 것은 가중치가 일정한 최단경로를 구하는 것이므로 BFS를 이용했다.
- 승객을 찾는 함수 findP
    - 연료가 없을 땐 탐색을 종료한다.
    - 현재 택시 위치에 승객이 있을 수 있음에 유의한다.
    - 승객을 태우고 map의 해당 칸을 0으로 바꾸어 승객의 탐승을 표현한다.
    - 같은 거리라면 우선순위가 존재하기 때문에 동일 거리를 관리할 수 있도록 qsize만큼만을 확인할 수 있는 while문을 사용하였다.
- 승객을 목적지로 데려가는 함수 toDst
    - 연료가 없을 땐 탐색을 종료한다.
    - 승객의 출발점과 도착점이 같을 수 있음에 유의한다.
    - 목적지에 도달하면 이동 거리에 따라 연료를 계산한다.
