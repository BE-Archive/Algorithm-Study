## 문제 풀이
- 전체 맵이 크지않고, 병사를 3명만 놓는 것으로 고정되어 있다.
- 병사의 위치를 next permutaion으로 지정하고, 해당 조건에서 실행할 때 몇 명을 잡을 수 있는지를 계산해야 한다.
- 어느 적을 공격하는지를 까다롭게 결정해야 한다.
  - 같은 적이 여러 궁수에게 공격당할 수 있다
  - 가장 가까운 적을 공격하고, 그러한 적이 여럿이면 가장 왼쪽의 적을 공격한다.
