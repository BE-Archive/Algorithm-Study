## 풀이과정
- 0을 상하좌우로 움직이면서 퍼즐을 맞추어야 한다.
  - isValid를 통해 움직일 수 있는 방향인지 체크하며 움직인다.
  - 이 때, 중복 연산을 피하기 위해 방문 확인을 해야 한다.
- 방문 확인
  - 각각의 상태는 tmp = tmp*10 + arr[i][j];를 통해서 해시를 생성하여 저장할 수 있다.
  - Hashset을 사용하여 tmp 값을 해시로 두어 방문처리 할 수 있다.
