...
1. 통나무는 위, 아래, 왼쪽, 오른쪽으로 이동이 가능하고, 최소 이동횟수 (회전 포함)을 찾아야 한다.

2. 가중치 없는 최단 -> BFS로 해결

3. 그렇다면 3개가 이어진 통나무를 어떻게 방문처리 해주어 최적화해야할까를 고민해야한다.

4. 통나무의 시작, 중간, 끝의 x,y좌표를 차원으로 만들면 [N][N][N][N][N][N]로 50^6이 되어 안된다!.

5. 그렇다면 통나무의 중간만 방문처리하며 현재 통나무가 가로 방향인지, 세로 방향인지만 관리해주자!.

6. 이후, BFS를 통해 start, middle, end의 3위치가 모두 경계를 나가지 않는지, 1에 닿는지를 확인해준다.

   > notValid() 메서드에서 이를 확인 하나라도 경계를 나가거나 1에 만나면 갈 수 없음

7. 만약 갈 수 있다면 큐에 넣고 현재 통나무의 방향 (가로, 세로)에 visit처리 해준다.

8. 이후, 현재 통나무를 회전시켜본다.

   > Tree객체가 자신의 방향을 가지고 있으므로 이를 통해 가로 -> 세로, 세로 -> 가로로 회전 시켜본다.  
   > 중앙 제외 8방향 확인을 통해 회전이 가능하면 회전한 결과를 큐에 넣는다.

9. BFS를 너비만큼 탐색하여 목적지에 도착하면 너비를 출력한다.

10. 이때, 목적지에 도착한 것을 확인할 때, 회전 도중, start, end가 뒤집혀
    > tree.start != dest.start이지만, tree.end == dest.start가 되는 경우가 있을 것 같아 이를 추가
